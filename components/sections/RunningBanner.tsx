...
export function RunningBanner() {
  ...
  return (
    ...
    //                 width={70}
    //                 height={70}
    //                 className="rounded-full object-cover align-center"
    //               />
    //             ) : (
    //               <Bike className="mr-1" />
    //             )}
    ...
    //               <span className='truncate'>{currentDrive.meetingPoint}</span>
    //             </div>
    //             <div className="flex items-center space-x-1">
    ...
    //                 <span className='font-medium truncate'>{currentDrive.description}</span>
    //               </div>
    //             )}
    //           </div>
    //         </div>
    ...
    //   </div>
    // </div>
  <div className="bg-white dark:bg-gray-900/80 backdrop-blur-md rounded-xl shadow-lg relative overflow-hidden p-4">
    {/* Drive Container */}
      ...
      <div className="flex items-start md:items-center gap-4 w-full md:w-2/3">
        ...
        <div className="flex flex-col gap-1 flex-1">
          <div className="flex flex-wrap items-center gap-2">
            ...
            {daysUntil <= 3 && (
              <Badge className="animate-pulse text-xs flex items-center gap-1 bg-yellow-100 text-yellow-800 dark:bg-yellow-900 dark:text-yellow-200">
          ...
          <div className="flex flex-wrap items-center gap-3 ">
            <span className="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-300">
              ...
              Location : {currentDrive.location}
            </span>
            <span className="flex items-center gap-3 text-sm text-purple-600 dark:text-yellow-600">
              <MapPin className="h-4 w-4" /> Meeting Point & Time : {currentDrive.meetingPoint}
            </span>
            <span className="flex items-center gap-3 text-sm text-purple-600 dark:text-yellow-600">
              <MapPin className="h-4 w-4" /> End Point : {currentDrive.endingPoint}
            </span>
            <span className="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-300">
              <Calendar className="w-4 h-4" />
              {formatDate(currentDrive.date)}
            </span>
            <span className="flex items-center gap-1 text-sm text-gray-600 dark:text-gray-300">
              <Clock className="w-4 h-4" />
              {daysUntil < 0 ? 'Drive Completed' : daysUntil === 0 ? 'Today!' : `${daysUntil} days to go`}
            </span>
            <span className="flex items-center gap-1 text-sm text-gray-600 dark:text-gray-300">
              <Users className="w-4 h-4" />
              Riders :  {currentDrive.participants} 
            </span>
            <span className="flex items-center gap-1 text-sm text-gray-600 dark:text-gray-300">
              <TreePine className="w-4 h-4" />
              Trees Target : {currentDrive.treesTarget} 
            </span>
            {currentDrive.duration && (
              <span className="flex items-center gap-1 text-sm text-gray-600 dark:text-gray-300">
                <Clock className="w-4 h-4" />Duration : {currentDrive.duration} hours
              </span>
            )}
          </div>

          {/* Description */}
          <p className="text-sm mt-1 text-sm text-gray-600 dark:text-gray-300 line-clamp-3 sm:line-clamp-1">
            Description : {currentDrive.description}
          </p>
          {currentDrive.organizer && (
            <div className="text-sm text-sm text-gray-600 dark:text-gray-300 mt-1">
              Organized by : 
              <span className="font-medium"> {currentDrive.organizer}</span>
            </div>
          )}
        ...
        {currentDrives.map((_, index) => (
          <button
            ...
            className={`w-2 h-1 rounded-full transition-all duration-200 ${
              index === currentIndex
                ? 'bg-green-600 w-6 dark:bg-blue-600 hover:bg-green-600 dark:hover:bg-blue-600 hover:w-6' : 'bg-blue-600 dark:bg-green-600 hover:bg-blue-600 dark:hover:bg-green-600 hover:w-6'
            }`}
            aria-label={`Go to drive ${index + 1}`}
          />
        ))}
